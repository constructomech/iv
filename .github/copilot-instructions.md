...
Never pipe build or test commands through filters (e.g. `| Select-String`, `| Select-Object`, `| grep`). Run the command directly so progress is visible in the terminal, then grep the output afterwards if needed.
